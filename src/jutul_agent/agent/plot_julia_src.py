...
# CairoMakie's own PNG default, so rendering the poster elsewhere keeps it at the
# resolution the record has always had.
POSTER_PX_PER_UNIT = 2
...
def _poster_block(png_path: Path, *, html_fallback: Path | None) -> str:
    ...
    is given, to export a self-contained WebGL page instead if no rasteriser could
    ...
    The poster is rendered offscreen by GLMakie, the same rasteriser the browser's
    WebGL view uses, so the durable record shows what the session showed. CairoMakie
    renders the same 3D figure around twenty times slower and, having no depth test,
    resolves near-coplanar faces differently from either GL backend: the prettier
    picture and the less faithful one. It stays as the fallback, for a session with no
    GL context at all, where the choice is a Cairo poster or none.

    The GL screen is closed rather than left on the figure, which also takes it off the
    scene tree; the Cairo fallback needs ``DETACH_CAIRO_SCREENS`` for that.
    """
    ...
    return (
        ...
        "        try\n"
        f'            GLMakie.save(raw"{png_path.as_posix()}", _fig,\n'
        f"                px_per_unit = {POSTER_PX_PER_UNIT})\n"
        "        finally\n"
        "            GLMakie.closeall()\n"
        "        end\n"
        "    catch\n"
